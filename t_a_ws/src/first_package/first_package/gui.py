...
class GridMap(QtWidgets.QWidget):
        ...
        if event.button() == QtCore.Qt.RightButton:
            for qrectf in self.qrectfs:
                if qrectf.contains(event.pos()):
                    ...
                    self.simulator.gui_node.publish_border_pose(x, y)
    ...
    def __fillTiles(self, painter: QtGui.QPainter) -> None:
        ...
        # logging.info(self.walls)
        if self.if_needed_send_json: 
            listik = str(self.walls.copy())
            msg = String()
            msg.data = listik
            # self.path_pub.publish(msg)
            self.simulator.gui_node.publish_json_borders(msg)
            self.if_needed_send_json = False

    def __drawPath(self, painter: QtGui.QPainter):
        
        for listos in self.simulator.gui_node.path:
            indexik = self.simulator.gui_node.path.index(listos)
            pen = QtGui.QPen(self.simulator.gui_node.colors[indexik], 10)
            painter.setPen(pen)
            for pointik in list(eval(listos)):
                painter.drawPoint(int(pointik[0]*self.tile_size + self.tile_size/2), int(pointik[1]*self.tile_size + self.tile_size/2))
...
class GuiNode(Node):
    def __init__(self):
        ...
        self.border_publisher = self.create_publisher(Pose, 'border', 10)

        self.json_borders_pub = self.create_publisher(String,'json_borders',10)

        self.path_subscription = self.create_subscription(String,'path_pub', self.path_listener_callback,10)
        self._data = None
    ...
    def path_listener_callback(self, msg):
        ...
        self.colors.append(QtGui.QColor(random.randint(0,255), random.randint(0,255), random.randint(0,255), 255))
    ...
    def publish_border_pose(self, x, y):
        goal_pose = Pose()
        goal_pose.x = float(x)
        goal_pose.y = float(y)
        goal_pose.theta = 0.0
        self.border_publisher.publish(goal_pose) 

    def publish_json_borders(self, data):
        self.json_borders_pub.publish(data) 
